# otto-dances
Otto Robot Movement Database

## 📖 Proje Açıklaması

Bu proje, Otto Robot için kapsamlı bir online hareket veritabanı içerir. ESP8266 tabanlı Otto robotlarınız için 10 farklı kategoride 150 hareket sunmaktadır.

## 📚 Kategoriler

Bu veritabanı aşağıdaki 10 kategoriyi içerir:

| Kategori | İkon | Açıklama | Hareket Sayısı |
|----------|------|----------|----------------|
| Dances | 💃 | Popüler dans hareketleri | 15 |
| Music | 🎵 | Müzik senkronizasyonlu hareketler | 40 |
| Emotions | 😊 | Duygusal ifadeler | 15 |
| Gestures | 👋 | Jest ve işaretler | 15 |
| Actions | 🚶 | Temel hareketler | 15 |
| Sports | ⚽ | Spor hareketleri | 15 |
| Animals | 🐕 | Hayvan taklitleri | 15 |
| Games | 🎮 | Oyun hareketleri | 15 |
| Exercises | 💪 | Egzersiz rutinleri | 15 |
| Sequences | 🔗 | Hareket zincirleri | 15 |

## 🎭 Danslar (Dances)

| ID | İsim | İkon |
|---|---|---|
| gangnam | Gangnam Style | 🕺 |
| moonwalk | Moonwalk | 🌙 |
| floss | Floss | 🎮 |
| macarena | Macarena | 💃 |
| thriller | Thriller | 🧟 |
| russian | Russian Kick | 🦵 |
| chicken | Chicken Dance | 🐔 |
| dab | Dab | 🙆 |
| robot | Robot | 🤖 |
| shake | Shake | 🫨 |
| twerk | Twerk | 🍑 |
| wave | Wave | 👋 |
| disco | Disco | 🪩 |
| salsa | Salsa | 🌶️ |
| hype | Hype | 🔥 |

## 🎵 Müzik (Music)

### Pop/Dance Hits

| ID | Song | Artist | BPM | Icon |
|---|---|---|---|---|
| despacito | Despacito | Luis Fonsi | 89 | 💃 |
| shape_of_you | Shape of You | Ed Sheeran | 96 | 🎶 |
| uptown_funk | Uptown Funk | Bruno Mars | 115 | 🕺 |
| blinding_lights | Blinding Lights | The Weeknd | 171 | ✨ |
| levitating | Levitating | Dua Lipa | 103 | 🌟 |
| bad_guy | Bad Guy | Billie Eilish | 135 | 😈 |
| old_town_road | Old Town Road | Lil Nas X | 136 | 🤠 |
| drivers_license | Drivers License | Olivia Rodrigo | 144 | 🚗 |
| dance_monkey | Dance Monkey | Tones and I | 98 | 🐵 |
| señorita | Señorita | Shawn Mendes | 117 | 🌹 |

### Rock/Classic Rock

| ID | Song | Artist | BPM | Icon |
|---|---|---|---|---|
| bohemian_rhapsody | Bohemian Rhapsody | Queen | 144 | 👑 |
| sweet_child | Sweet Child O' Mine | Guns N' Roses | 125 | 🎸 |
| smells_like_teen | Smells Like Teen Spirit | Nirvana | 117 | 🎤 |
| back_in_black | Back in Black | AC/DC | 93 | ⚡ |
| livin_on_prayer | Livin' on a Prayer | Bon Jovi | 123 | 🙏 |
| eye_of_tiger | Eye of the Tiger | Survivor | - | 🐯 |
| we_will_rock_you | We Will Rock You | Queen | - | 👊 |

### Kids/Family Songs

| ID | Song | Artist | BPM | Icon |
|---|---|---|---|---|
| baby_shark | Baby Shark | Kids Song | 110 | 🦈 |
| frozen_let_it_go | Frozen - Let It Go | Disney | 137 | ❄️ |
| hakuna_matata | Hakuna Matata | Lion King | 125 | 🦁 |
| under_the_sea | Under the Sea | Little Mermaid | 180 | 🐠 |
| wheels_on_bus | Wheels on the Bus | Kids Song | 120 | 🚌 |
| old_macdonald | Old MacDonald Had a Farm | Kids Song | 120 | 🚜 |
| whole_new_world | A Whole New World | Aladdin | 126 | 🧞 |
| happy_birthday | Happy Birthday | Traditional | 120 | 🎂 |
| jingle_bells | Jingle Bells | Traditional | - | 🔔 |

### Movie/TV Themes

| ID | Song | Artist | BPM | Icon |
|---|---|---|---|---|
| mission_impossible | Mission Impossible Theme | MI | 160 | 🕵️ |
| pink_panther | Pink Panther Theme | Pink Panther | 115 | 🐆 |
| james_bond | James Bond Theme | 007 | 138 | 🔫 |
| ghostbusters | Ghostbusters Theme | Ghostbusters | 116 | 👻 |
| axel_f | Axel F | Beverly Hills Cop | 125 | 🎹 |
| starwars_theme | Star Wars Theme | Star Wars | - | ⭐ |
| pirates_caribbean | Pirates of the Caribbean | POTC | - | 🏴‍☠️ |
| avengers_theme | Avengers Theme | Marvel | - | 🦸 |
| imperial_march | Imperial March | Star Wars | - | 👤 |
| cantina_band | Cantina Band | Star Wars | - | 🎺 |

### Game/Video Game Music

| ID | Song | Artist | BPM | Icon |
|---|---|---|---|---|
| tetris_theme | Tetris Theme | Tetris | - | 🎮 |
| mario_theme | Mario Theme | Super Mario | - | 🍄 |
| zelda_theme | Zelda Theme | Legend of Zelda | - | 🗡️ |
| pokemon_theme | Pokemon Theme | Pokemon | - | ⚡ |


## 😊 Duygular (Emotions)

| ID | İsim | İkon |
|---|---|---|
| happy | Happy | 😊 |
| sad | Sad | 😢 |
| angry | Angry | 😠 |
| excited | Excited | 🤩 |
| scared | Scared | 😨 |
| sleepy | Sleepy | 😴 |
| love | Love | 😍 |
| confused | Confused | 😕 |
| surprised | Surprised | 😲 |
| proud | Proud | 😤 |
| shy | Shy | 😳 |
| bored | Bored | 😑 |
| curious | Curious | 🤔 |
| determined | Determined | 😤 |
| relieved | Relieved | 😌 |

## 👋 Jestler (Gestures)

| ID | İsim | İkon |
|---|---|---|
| hello | Hello | 👋 |
| goodbye | Goodbye | 👋 |
| yes | Yes | ✅ |
| no | No | ❌ |
| clap | Clap | 👏 |
| point_left | Point Left | 👈 |
| point_right | Point Right | 👉 |
| bow | Bow | 🙇 |
| shrug | Shrug | 🤷 |
| thumbs_up | Thumbs Up | 👍 |
| wave_both_hands | Wave Both Hands | 🙌 |
| come_here | Come Here | 🫱 |
| stop | Stop | ✋ |
| thinking | Thinking | 🤔 |
| celebrate | Celebrate | 🎉 |

## 🚶 Hareketler (Actions)

| ID | İsim | İkon |
|---|---|---|
| walk_forward | Walk Forward | 🚶 |
| walk_backward | Walk Backward | 🚶 |
| turn_left | Turn Left | ↪️ |
| turn_right | Turn Right | ↩️ |
| jump | Jump | 🦘 |
| sit | Sit | 🪑 |
| standup | Stand Up | 🧍 |
| lean_left | Lean Left | ↖️ |
| lean_right | Lean Right | ↗️ |
| balance | Balance | ⚖️ |
| spin_360 | Spin 360 | 🔄 |
| tiptoe | Tiptoe | 🩰 |
| crouch | Crouch | 🧎 |
| stretch | Stretch | 🤸 |
| wobble | Wobble | 🫨 |

## ⚽ Sporlar (Sports)

| ID | İsim | İkon |
|---|---|---|
| kick | Kick | 🦶 |
| punch | Punch | 👊 |
| celebrate_goal | Celebrate Goal | ⚽ |
| yoga_tree | Yoga Tree Pose | 🧘 |
| pushup | Push Up | 💪 |
| basketball_shoot | Basketball Shoot | 🏀 |
| golf_swing | Golf Swing | ⛳ |
| tennis_serve | Tennis Serve | 🎾 |
| boxing_combo | Boxing Combo | 🥊 |
| karate_stance | Karate Stance | 🥋 |
| football_dribble | Football Dribble | ⚽ |
| volleyball_spike | Volleyball Spike | 🏐 |
| baseball_pitch | Baseball Pitch | ⚾ |
| swimming_stroke | Swimming Stroke | 🏊 |
| victory_pose | Victory Pose | 🏆 |

## 🐕 Hayvanlar (Animals)

| ID | İsim | İkon |
|---|---|---|
| dog | Dog | 🐕 |
| cat | Cat | 🐈 |
| bird | Bird | 🐦 |
| penguin | Penguin | 🐧 |
| dinosaur | Dinosaur | 🦖 |
| crab | Crab | 🦀 |
| monkey | Monkey | 🐒 |
| elephant | Elephant | 🐘 |
| rabbit | Rabbit | 🐇 |
| snake | Snake | 🐍 |
| frog | Frog | 🐸 |
| kangaroo | Kangaroo | 🦘 |
| chicken | Chicken | 🐔 |
| bear | Bear | 🐻 |
| butterfly | Butterfly | 🦋 |

## 🎮 Oyunlar (Games)

| ID | İsim | İkon |
|---|---|---|
| rock_paper_scissors | Rock Paper Scissors | ✊ |
| dice_roll | Dice Roll | 🎲 |
| coin_flip | Coin Flip | 🪙 |
| hide_seek | Hide and Seek | 👀 |
| simon_says | Simon Says | 🗣️ |
| freeze_dance | Freeze Dance | 🧊 |
| musical_chairs | Musical Chairs | 🪑 |
| hopscotch | Hopscotch | 🔢 |
| limbo | Limbo | 🎯 |
| red_light_green_light | Red Light Green Light | 🚦 |
| follow_leader | Follow the Leader | 👣 |
| guess_number | Guess the Number | 🔮 |
| magic_trick | Magic Trick | 🎩 |
| peek_a_boo | Peek-a-Boo | 👻 |
| spin_bottle | Spin the Bottle | 🍾 |

## 💪 Egzersizler (Exercises)

| ID | İsim | İkon |
|---|---|---|
| morning_stretch | Morning Stretch | 🌅 |
| warmup | Warm Up | 🔥 |
| cooldown | Cool Down | ❄️ |
| cardio_blast | Cardio Blast | 💨 |
| leg_day | Leg Day | 🦵 |
| arm_workout | Arm Workout | 💪 |
| core_strength | Core Strength | 🎯 |
| flexibility | Flexibility | 🤸 |
| balance_training | Balance Training | ⚖️ |
| quick_energizer | Quick Energizer | ⚡ |
| desk_break | Desk Break | 💺 |
| bedtime_relaxation | Bedtime Relaxation | 🌙 |
| power_workout | Power Workout | 🔋 |
| gentle_movement | Gentle Movement | 🕊️ |
| full_body | Full Body | 🏋️ |

## 🔗 Sekanslar (Sequences)

| ID | İsim | İkon |
|---|---|---|
| dance_party | Dance Party | 🎉 |
| morning_routine | Morning Routine | 🌅 |
| show_time | Show Time | 🎭 |
| workout_combo | Workout Combo | 💪 |
| story_time | Story Time | 📖 |
| animal_parade | Animal Parade | 🦁 |
| emotion_rollercoaster | Emotion Rollercoaster | 🎢 |
| sports_medley | Sports Medley | 🏅 |
| music_mix | Music Mix | 🎵 |
| game_night | Game Night | 🎮 |
| quick_demo | Quick Demo | ⚡ |
| full_performance | Full Performance | 🌟 |
| educational_sequence | Educational Sequence | 📚 |
| party_mode | Party Mode | 🎊 |
| random_fun | Random Fun | 🎲 |


## 🚀 Kullanım Talimatları

### Kategori Listesi Alma
```
GET /index.json
```

### Belirli Bir Kategorideki Hareketleri Listeleme
```
GET /{category}/index.json
```

Örnek:
```
GET /dances/index.json
GET /music/index.json
```

### Belirli Bir Hareketi Alma
```
GET /{category}/{movement_id}.json
```

Örnek:
```
GET /dances/gangnam.json
GET /music/happy_birthday.json
GET /emotions/happy.json
```

## 📁 Dosya Formatları

### Dans/Hareket Formatı

```json
{
  "name": "Hareket Adı",
  "repeat": 3,
  "steps": [
    [servo0, servo1, servo2, servo3, delay_ms],
    ...
  ]
}
```

### Müzik Formatı

```json
{
  "name": "Şarkı Adı",
  "bpm": 120,
  "duration_seconds": 20,
  "sync_to_beat": true,
  "steps": [...]
}
```

### Duygu Formatı

```json
{
  "name": "Duygu Adı",
  "emotion": "joy/sad/anger/etc",
  "intensity": "low/medium/high",
  "steps": [...]
}
```

### Jest Formatı

```json
{
  "name": "Jest Adı",
  "type": "greeting/response/indication",
  "steps": [...]
}
```

### Hareket Formatı

```json
{
  "name": "Hareket Adı",
  "type": "locomotion/posture/balance",
  "distance_cm": 10,
  "steps": [...]
}
```

### Spor Formatı

```json
{
  "name": "Spor Hareketi Adı",
  "sport": "football/basketball/etc",
  "difficulty": "easy/medium/hard",
  "steps": [...]
}
```

### Hayvan Formatı

```json
{
  "name": "Hayvan Adı",
  "animal": "species",
  "characteristic_move": "walk/hop/slither",
  "steps": [...]
}
```

### Oyun Formatı

```json
{
  "name": "Oyun Adı",
  "game": "game_type",
  "random": true/false,
  "steps": [...]
}
```

### Egzersiz Formatı

```json
{
  "name": "Egzersiz Adı",
  "duration_minutes": 5,
  "difficulty": "easy/medium/hard",
  "target_area": "full_body/legs/arms/core",
  "steps": [...]
}
```

### Sekans Formatı

```json
{
  "name": "Sekans Adı",
  "type": "playlist/routine/performance",
  "total_duration_seconds": 60,
  "sequence": [
    {"category": "dances", "id": "gangnam", "repeat": 2},
    {"category": "emotions", "id": "happy", "repeat": 1}
  ]
}
```


### Alanlar

| Alan | Açıklama |
|---|---|
| name | Hareketin görünen adı |
| repeat | Hareketin kaç kez tekrarlanacağı (opsiyonel) |
| steps | Hareket adımları dizisi |

### Step Formatı

Her step 5 değerden oluşur:
- `servo0`: Sol bacak servo açısı (0-180)
- `servo1`: Sağ bacak servo açısı (0-180)
- `servo2`: Sol ayak servo açısı (0-180)
- `servo3`: Sağ ayak servo açısı (0-180)
- `delay_ms`: Adım süresi (milisaniye)

### Servo Konfigürasyonu

- **servo0**: Sol bacak (kalça)
- **servo1**: Sağ bacak (kalça)
- **servo2**: Sol ayak (ayak bileği)
- **servo3**: Sağ ayak (ayak bileği)
- **Nötr pozisyon**: [90, 90, 90, 90]
- **Güvenli aralık**: Genellikle 45-135 derece

## ➕ Yeni Hareket Ekleme Rehberi

1. İlgili kategori klasöründe yeni bir JSON dosyası oluşturun (örn: `{category}/yeni_hareket.json`)

2. Hareket dosyasını yukarıdaki formatlara göre doldurun

3. İlgili kategorinin `index.json` dosyasına yeni hareketi ekleyin:
```json
{"id": "yeni_hareket", "name": "Yeni Hareket", "icon": "🎵"}
```

4. Ana `index.json` dosyasındaki hareket sayısını güncelleyin


## 📡 ESP8266 Bağlantı Bilgisi

Otto robotunuzda bu veritabanını kullanmak için:

```cpp
#include <ESP8266WiFi.h>
#include <ESP8266HTTPClient.h>
#include <ArduinoJson.h>

const char* baseUrl = "https://raw.githubusercontent.com/muradaras161616/otto-dances/main/";

// Kategori listesini al
String getCategoryList() {
  HTTPClient http;
  http.begin(String(baseUrl) + "index.json");
  int httpCode = http.GET();
  String payload = http.getString();
  http.end();
  return payload;
}

// Belirli bir kategorideki hareketleri al
String getCategoryMovements(String category) {
  HTTPClient http;
  http.begin(String(baseUrl) + category + "/index.json");
  int httpCode = http.GET();
  String payload = http.getString();
  http.end();
  return payload;
}

// Belirli bir hareketi al
String getMovement(String category, String movementId) {
  HTTPClient http;
  http.begin(String(baseUrl) + category + "/" + movementId + ".json");
  int httpCode = http.GET();
  String payload = http.getString();
  http.end();
  return payload;
}
```

## 📜 Lisans

MIT License
